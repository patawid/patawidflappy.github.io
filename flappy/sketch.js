...
let SPACE = 32
let NIGHT_SCROLL_SPEED = 1
let NIGHT_LOOPING_SPEED = 438
...
let bgImage, groundImage, birdImage, bird, pipeImage, pipe, lastY, nightImage, currentImage
...
function title() {
    ...
    textAlign(CENTER)
    textFont(flappyFont)
    text("Night Night Bird", width / (2 * 1.74), 100)
    textSize(14)
...
function draw() {
    ...
    if (image(nightImage, -bgScroll, 0)) {
        bgScroll = (bgScroll + BACKGROUND_SCROLL_SPEED) % BACKGROUND_LOOPING_PT

        //setInterval(draw, 3000)
    }

    // image(currentImage, -bgScroll, 0)
    // bgScroll = (bgScroll + BACKGROUND_SCROLL_SPEED) % BACKGROUND_LOOPING_PT


//    image(nightImage, - nightScroll, 0)
//    nightScroll = (nightScroll + NIGHT_SCROLL_SPEED) % NIGHT_LOOPING_SPEED
...
function done() {
    ...
    if (points > 0) {
        ...
        text("noo! You Lost", width/(2 * 1.74), 100)
    }
    textSize(14)
    text("Score: " + points, width/(2 * 1.74), 150)
    text("Press Enter to Try Again", width/(2 * 1.74), 200)
}
